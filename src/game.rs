...
pub fn launch_game(dx_level: Renderer, use_avx: bool, args: Vec<String>) -> Result<()> {
    ...
    match (dx_level, use_avx) {
        ...
        (Renderer::DX9, false) => cwd.push("bin\\AnomalyDX9.exe"),
        (Renderer::DX9, true) => cwd.push("bin\\AnomalyDX9AVX.exe"),
        (Renderer::DX10, false) => cwd.push("bin\\AnomalyDX10.exe"),
        (Renderer::DX10, true) => cwd.push("bin\\AnomalyDX10AVX.exe"),
        (Renderer::DX11, false) => cwd.push("bin\\AnomalyDX11.exe"),
        (Renderer::DX11, true) => cwd.push("bin\\AnomalyDX11AVX.exe"),
    };
    if !cwd.exists() {
